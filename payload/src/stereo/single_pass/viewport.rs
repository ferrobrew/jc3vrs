...
#[derive(Clone, Copy)]
pub(super) struct EngineContext(&'static jc3gi::graphics_engine::device::Context);

impl EngineContext {
    /// The engine's immediate context, or `None` if the device/context is not live yet.
    pub(super) fn get() -> Option<Self> {
    ...
    pub(super) fn with_lock<R>(self, f: impl FnOnce(&ID3D11DeviceContext) -> R) -> R {
