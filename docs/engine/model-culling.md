...
`m_SpawnBFBCParams` `+0x170`, `m_SpawnBFBCState` `+0x178`, `m_ViewPos` `+0x190`, `m_ViewDir` `+0x19C`)
builds its own BFBC params against `m_ActiveCamera` and uses them in `CSpawnFactoryImpl::CheckInternal`
(`0x140_F09_430`) as a "don't (de)spawn while visible" gate. Its budgets are characters and vehicles, so
it governs NPC/vehicle (de)spawns near the view edge, not buildings.
