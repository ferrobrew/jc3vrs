...
Put the panel at roughly 1.8 m (inside the 1.5–2.5 m comfort band — far enough to avoid vergence strain, near enough to read), around 45–55° wide, flat. The panel eases toward the head's current orientation on all three axes — yaw, pitch, and roll — with no deadzone. Roll matters for JC3: parachute, wingsuit, and vehicle sections all bank heavily, and VR head-tracking adds more. Without deadzone the panel always tracks the head; the halflife alone controls how snappy the follow is. A quick flick barely moves the panel because the halflife limits how far it can travel in a short interval, but the panel never gets stuck at a permanent offset from where the head is looking. Position is just de-jittered.

Use a critically-damped exponential so it converges fast with no overshoot, and so it's frame-rate independent: `alpha = 1 - 2^(-dt/halflife); current += (target - current) * alpha` (Holden's damper / Unity SmoothDamp). Yaw uses the shortest angular path (`shortest_angle_delta`) so the panel doesn't swing the long way around when the head crosses the ±180° boundary. Sensible starting halflives are yaw 0.15 s, pitch 0.3 s, roll 0.2 s, position 0.1 s. Expose all of them as sliders — these are starting points to feel out in the preview, not settled values.
