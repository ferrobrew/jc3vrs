...
What's left is the VR runtime and the remaining input and scale work: the OpenXR session, real HMD pose, per-eye off-axis projection, and per-eye resolution — the largest remaining piece, since today's build is a desktop stereo prototype with no headset path yet — plus the VR-controller write path and dropping FSR's render scale below 1:1.
