...
This document records how FSR is integrated: which version, where it dispatches, and the order it was built in. The concrete reverse-engineering it leans on lives in `rendering.md` (the post-effects chain, the buffers, the stereo double-dispatch) and the pyxis-defs.

## Why FSR, and which version

The prior AA survey ruled out the alternatives for a mod shipping into a third-party game on mixed GPUs. DLSS is NVIDIA-only and its license is hostile to this use case; XeSS's cross-vendor path is DX12-only (its DX11 backend is Intel-Arc-only). FSR 4 is disqualifying: it is an ML model gated to AMD RDNA4 (with later INT8 backports to RDNA3/RDNA2), so it never runs on NVIDIA or Intel — a non-starter for a mod that has to run on whatever GPU the player owns. FSR is MIT-licensed, runs on every vendor, and has a DirectX 11 path. That license and cross-vendor reach are the whole reason it wins.

**The mod ships FSR 2.2.1**, via the MIT community DX11 port (`optiscaler/FidelityFX-FSR2-DX11`), wrapped by the `fsr-sys` crate. FSR 3.1 was the initial target — it is the newest version still on plain SM6.2 compute with no ML hardware gate, MIT, with an explicit native-AA mode and the newer `ffxapi` interface — but its DX11 backend is community-maintained and the FSR 2.2.1 DX11 port is what proved stable under Proton, so that is what shipped. The 2.2 API is clunkier and its native-AA setup is manual, but the reconstruction quality at 1:1 is what matters here and the port loads and dispatches reliably.
...
## Where it dispatches: post-tonemap

The mod dispatches FSR post-tonemap, at the engine's anti-aliasing hook. There were two candidate dispatch points; this one is the SMAA drop-in.

**Post-tonemap, at the anti-aliasing hook (the shipped path).** SMAA already runs at step 9 of the post chain (rendering §3), after the HDR→LDR tonemap at step 7, on the LDR temp texture in the manager's three-slot ring. FSR drops in at the existing `anti_aliasing_apply` hook and is fed that same LDR image, in exactly the slot SMAA reads. No HDR flag, no exposure wiring — the image is already exposed. The mechanical cost is mimicking the slot-ring handoff (read slot N at `mgr + slot + 80/83`, write the result into the next slot, advance the index — the engine AA does precisely this), and a modest quality cost: FSR reconstructs already-tonemapped color, so it can't recover highlight detail the tonemapper already compressed. Effects after step 9 (sun halo, fade) then render on top un-anti-aliased, but those are full-screen overlays (an additive bloom and a color multiply), so it doesn't matter.

**Pre-tonemap, at the post-chain entry (the deferred alternative).** FSR's canonical position is on HDR pre-tonemap color, where its highlight reconstruction has the full dynamic range to work with. Dispatching at the start of the post block (the `capture_main_color` seam, where MainColor still holds the clean HDR scene), resolving into MainColor in place, and letting the rest of the chain tonemap the cleaned image is the better-quality input, at the cost of wiring the HDR and auto-exposure flags and using a less conventional injection point. This path is not yet built (see order-of-work step 6).

Note that FSR's usual "before tonemap" placement is driven mostly by *upscaling*: an upscaler must run before resolution-dependent effects so they execute once at display resolution. That argument does not apply at native AA res, so it does not force the early position here; the remaining pre-tonemap argument is purely the highlight-quality one, which is modest.

The engine's own AA is suppressed wherever FSR is active: the `force_smaa_1x` logic also drops `CAntiAliasingEffect::m_Mode` to off (mode 0 is the passthrough branch in `Apply`).
...
The crucial fact: **FSR's AA mode and upscaling mode are the same context, the same inputs, the same dispatch.** The only differences are that `renderSize < displaySize` instead of equal, and that the scene must actually be rendered at the lower per-eye resolution. So the integration built for AA *is* the upscaler, minus the resolution plumbing — there is no throwaway work. Accordingly:

1. The integration carries `renderSize` and `displaySize` as explicit parameters; they are not hardcoded equal. Native AA is just the first configuration of an upscaler.
2. Native AA (1:1) ships now: testable on the desktop preview, no dependency on the resolution re-init, and it replaces SMAA directly.
3. When the VR runtime and per-eye resolution re-init land (the VR-runtime task's blocker 2 — re-driving `CreateRenderSetups` / `Graphics::Reset` at a chosen per-eye resolution), dropping `renderSize` below `displaySize` turns it into an upscaler with no FSR-side rework, just a render-scale slider. Not yet built (order-of-work step 7).
...
### VR-specific upscaling caveats (for when render scale drops below 1:1)
...
**The MV jitter cancellation.** A second contamination rides on the same vectors: the engine measures `curUV` under the *jittered* projection (the mod's FSR Halton jitter) against an unjittered previous VP, so every stored vector carries the current frame's sub-pixel jitter as a constant offset, while FSR expects jitter-free motion. Since jitter enters the VP as a constant NDC translation, the contamination is a constant per-frame UV offset known on the CPU; the decode subtracts it (`JitterUv`), using the current-vs-previous jitter delta when the stereo correction's (jittered) per-eye previous VP is the anchor. `config.fsr.mv_jitter_cancel`, default on (a no-op while jitter is off); the camera-side jitter sign convention and an amplitude scale are runtime-tunable alongside it (`fsr.jitter_sign` / `fsr.jitter_scale`). A correctness fix, not the issue-10 flicker — see *Jitter status* below.

**Jitter status: off by default.** The camera jitter excites a blob-scale, both-eyes, scene-side (pre-FSR MainColor) shadow-term flicker whose mechanism survived a long bisection (issue #10): the MV corrections above, the eye-1 post-chain dedupe, the jitter-unstable LOD dissolve patch, and a series of shadow-pipeline interventions (fit unjitter, pristine-camera restore, atlas freeze) all left it tracking `fsr.jitter` exactly, while exposure, the staged cascade constants, GI, SSAO, SSR, AO volumes, and occlusion queries were each ruled out empirically. FSR without jitter still resolves acceptably (no sub-pixel reconstruction, softer detail), so the default trades sharpness for stability: `fsr.jitter = false`. The bisection record lives in issue #10; re-enable the jitter to resume the hunt.
...
FSR is switchable on and off at runtime (`config.fsr.enabled`, default on), so it can be A/B'd against the engine's SMAA live in the preview. The toggle drives two coupled things together: whether the FSR dispatch runs, and whether the engine's own `CAntiAliasingEffect` is suppressed. Off means engine AA runs as normal and FSR does nothing; on means FSR runs and engine AA is dropped to off. The render-scale parameter (1:1 for now) sits alongside it, ready for the upscaling slider later. These live in the `FsrConfig` block with the other toggles.

The velocity encoding above was recovered by extracting FSR's reference shaders and JC3's own velocity shaders from `Shaders_F.shader_bundle` (an ADF container; shaders are named DXBC blobs laid out as `<name>\0…pad…DXBC<bytecode>`). The extraction recipe — slice by the `DXBC` magic + the size field at `+0x18`, disassemble via `D3DDisassemble` from the native `d3dcompiler_47.dll` under Wine — is documented in `shaders.md`, should other shaders need reading later.

## Risk status

- **DX11 port maturity under Proton/Wine.** Resolved by shipping the FSR 2.2.1 DX11 port rather than the FSR 3.1 one. The FSR 3.1 native DX11 backend is community-maintained and newer; the FSR 2.2.1 port builds, unpacks its shader permutations, and dispatches reliably under Proton, which is why it is what ships (see *Why FSR, and which version*).
- **Motion-vector convention and scale.** Resolved — see the *Motion vectors* section. The buffer is bias-encoded 8-bit (`(curUV-prevUV)*8`, clamped ±1, `*0.5+0.5`), so it needs a decode pass, not just a scale. The remaining empirical bit is FSR's sign/Y convention (visually obvious if wrong), exposed as `fsr.mv_sign` for live settling.
- **Reverse-Z / infinite-far flags.** Depth is reverse-Z near=1/far=0; FSR is told the depth is inverted (and infinite-far where applicable) or reprojection breaks.
- **Jitter ownership.** FSR needs the camera jittered by its own Halton sequence and the identical pixel offset fed to the dispatch, per eye per frame. The engine's 2-phase TAA jitter is the wrong sequence and is fully replaced (rendering §2.7 describes the projection-jitter injection point). The jitter itself ships off by default (`fsr.jitter = false`) — see *Jitter status* — so the replacement is exercised only when it is re-enabled.
- **Reactive / transparency masks.** Still open. Water, particles, and muzzle flashes can ghost without a reactive mask; a second pass is planned if transparencies smear.
