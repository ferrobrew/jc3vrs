...
**Mitigation (PLAN §5.3):** set the render context `m_Dt = 0` for the second eye, so dt-driven accumulators do not advance twice. (Exposure adaptation in the tonemap/histogram path is similarly dt/EMA-driven — see §5.4.)
...
- Auto-exposure EMA lives in the tonemapping effect (histogram generated in `GenerateHistogramForFinalScene`, applied in `DrawHistogramWindow`); it is dt-driven and runs per dispatch. Setting eye-1 dt to 0 (§5.2) also neutralizes double exposure adaptation. **[Partially verified — exposure EMA location inferred from the tonemap block, not line-traced.]**
...
- **VR UI compositing (floating quad):** hook `CUIManager::InitPlatformRT` and substitute your own offscreen RTV for `m_pDisplayRT` — the HUD then renders once into your texture (use as an `XrCompositionLayerQuad`) and never touches either eye's back buffer. Simpler alternative: gate the `StartRender`/`Submit` pair (`0x140F1B030`/`0x140F1B0D0`) so the HUD is emitted only for eye 1, and copy the HUD region into a layer.
