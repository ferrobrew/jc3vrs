...
- Auto-exposure is **frame-counted, not dt-driven** — `m_Dt = 0` does **not** fix it (dt feeds only motion blur). It is an **exposure-baked feedback loop**, and the stereo darkening it causes is **diagnosed but not yet solved** — see "the stereo darkening" below.

  The loop: `CToneMappingEffect::GenerateHistogramForFinalScene` meters luminance through a `"LuminanceToDepthWithExposure"` shader fed *last frame's* `m_CurrentExposure` (`ToneMappingEffect.m_CurrentExposure`) as a constant, so the metering is itself exposure-weighted. That feeds `CalculateMidAndBrightPointForHistogram` → `CToneMappingEffect::Update` (writes the new `m_CurrentExposure`) → next frame's metering. The luminance source is a fixed 320x180 RT written once in init, which is why `total_pixels` is identical stereo vs non-stereo. The final composite `DrawHistogramWindow` uses plain `"LuminanceToDepth"` (constant 1.0) — only `GenerateHistogramForFinalScene` re-meters. `Update` runs **once per real frame** (from `CPostEffectsManager::UpdateRender`); `GenerateHistogramForFinalScene` runs **per dispatch** (inside `CRenderBlockPostEffects::Draw`). `m_CurrentExposure`, the occlusion-query ring index and the histogram ping-pong selectors are single per-effect scalars shared by both eyes.

  **Open diagnosis (not a fix).** The obvious mitigation — meter once per frame, gate `GenerateHistogramForFinalScene` to one eye — is **exactly what the mod already does**: its detour returns early on eye 1, and the trace confirms eye 1 is skipped. Yet the same trace still shows the ~0.68x darkening *with that gate active*, and toggling the exposure gate changed nothing. So the residual perturbation is **not** eye-1's metering call (already killed); it is suspected to be the **async occlusion-query readback / ping-pong timing** desynced by the two-dispatch GPU pipeline depth (does the query ring index / ping-pong advance correctly when the scene dispatches twice but `Update` reads once — is the value `Update` consumes eye-0's, or stale/cross-frame?), or a separate exposure-bake on the visible scene render. **Unresolved.** The cheap disambiguator is a runtime A/B: with the gate active, force `m_CurrentExposure` to a fixed value (override `Update`'s write); if stereo then matches non-stereo brightness it is purely the loop value, otherwise there is a render dimming independent of exposure. The functions named here are all in `graphics_engine/tone_mapping.pyxis`. **[Mechanism verified from the symbol-bearing decompile; exact release addresses for the metering/exposure-write sites pending re-resolution — the split-dump is a different build.]**
...
- **Rotation / swap (in EVERY `GraphicsEngine::Draw` prologue):** `GraphicsEngine::Draw` calls the list-rotation driver `RotateRenderFrameData` at `Draw+0x1D0` (its call site is mislabeled `CKeep1000Frames::CKeep1000Frames` in this binary's symbols). It: (1) toggles a global 1-bit parity `dword_142ED7680` (`parity = (parity-1)&1`); (2) loops all 157 passes (via a sub mislabeled `std::map::operator[]`) calling each pass's `CRenderPass::SaveRenderFrameData` (vtable slot 3): `m_CurrentAddList = &m_Lists[parity]`, `m_CurrentDrawList = &m_Lists[(parity-1)&1]`, and **zeroes the new add-list's `m_NumElements`**; (3) flushes the overflow global back into the lists and resets its count. (Earlier drafts cited a `ToggleRenderpassLists` / a second `SaveRenderFrameData` at other addresses — those names/addresses do **not** exist in this build; they were hallucinated. The real per-pass swap symbol is `CRenderPass::SaveRenderFrameData`.)
...
**Two correct fixes:**
1. **Gate the rotation on eye 1** — what the mod does (`GATE_ROTATE_RENDER_FRAME_DATA`, default on): detour `RotateRenderFrameData` and skip it on the second dispatch. Eye 1 keeps eye 0's `m_CurrentDrawList` + counts and redraws the identical 194 blocks; also avoids double-toggling the parity and double-flushing the overflow.
2. **Run only the dispatch body twice** (`DispatchDraw`/`HandleDrawThreadTask`) under a single `GraphicsEngine::Draw` prologue — the rotation then runs once and both dispatches read the same populated buffer. More invasive; the mod instead runs the whole `Draw` twice and gates the per-dispatch side effects.
...
- **Render-list rotation `RotateRenderFrameData` (in the `Draw` prologue) -> skip on eye 1** — else eye 1's rotation flips every `m_CurrentDrawList` to the buffer eye 0 just zeroed and draws no scene geometry (§11). This is the core geometry fix under structure (A); not needed under (B).
