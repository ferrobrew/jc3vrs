...
On the camera-on-the-head question we take Boneworks' stance with one staged refinement. The character's head bone is the camera's source of truth, and we drive it toward the player's real head — hard in normal play, so it tracks 1:1 and stays comfortable (with no physics body yet, nothing pushes it off the real head). The win we keep from Boneworks is *releasing* that drive on genuine loss of control — ragdoll, knockdown, a scripted grab — so the camera rides your own body through the event; it reads as coherent and players brace for it. In our mechanism that release is just blending the head-bone override's stiffness to zero when the character enters such a state and letting the animation/physics take the head. We gate "release and ride" to clear, infrequent, full-loss-of-control states and stay glued to the real head through the frequent-but-minor perturbations (explosions, vehicle impacts, wingsuit turbulence), or it turns into nausea soup. The genuinely nausea-risky part — the camera being *pushed* by world collision during ordinary movement, Boneworks' signature and its signature sickness — only exists once there's a physics body to collide, which is deferred; we revisit that comfort tradeoff then (capsule-resist plus a fade versus full spring-push). Bonelab's biggest vehicle complaint is also instructive: never let the turn input bleed into steering, so keep view and steering decoupled.
