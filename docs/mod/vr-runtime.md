...
- `own_back_buffer` — substitute a mod-owned render target for the engine's swapchain-derived back buffer, so the DXGI swapchain stays at the window size instead of following the per-eye render resolution (default on; see [`swapchain-ownership.md`](swapchain-ownership.md)). The mirror's present becomes 1:1, and single-pass double-wide stops forcing a 2x-width swapchain.
