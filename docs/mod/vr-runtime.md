...
Rather than call `ApplyResize` directly, `resolution.rs` drives the engine's **own deferred display-mode state** — it writes the pending dimensions into `m_WindowWidth`/`m_WindowHeight` and sets `m_HasNewWindowSettings`, exactly as a windowed/settings resize does. The engine's `HandleModeChange`, serviced once per frame in the `Draw` prologue, then calls `ApplyResize` at the frame boundary the engine chose (previous dispatch drained, this frame not yet dispatched), so the idle-context assumption `ApplyResize` needs holds by construction (`docs/engine/rendering/render-setups-reinit.md` §2/§6). `ApplyResize` sets the camera aspect ratio and never touches the Win32 window (§4). It would also resize the DXGI swapchain buffers, but with `vr.own_back_buffer` on — the default — the mod substitutes that call so the swapchain stays at the window size while everything else follows the render size; see [`swapchain-ownership.md`](stereo/swapchain-ownership.md).
...
While a session runs the compositor owns the HMD present and the engine's own present is blocked for both eyes, so the game window would freeze on a stale frame. `mirror.rs` presents the game's own swapchain itself, once per frame, unsynced (`SyncInterval = 0`): a vsynced mirror on a 60 Hz monitor would throttle the whole loop, including the 90 Hz HMD submit, down to the monitor's refresh. It draws the configured eye (`vr.mirror_eye`) into the game back buffer, framed to the window aspect. With `vr.own_back_buffer` on the back buffer *is* the window client rect, so the present is 1:1 and the framing is simply a viewport. With it off the buffer is near-square at the per-eye resolution while the window keeps its 16:9 client rect, and the computed viewport pre-compensates DXGI's buffer→window stretch instead — the same computation, which degenerates to the identity when the two agree (unit-tested in `mirror.rs`).

`vr.mirror_framing` picks how the two aspects are reconciled. `Fill` (the default) scales the eye until it covers the window and crops the overflow, which is what other VR titles' desktop views do; the viewport it computes deliberately extends past the buffer edges, and since D3D11 rasterizes only within the render target, the crop falls out of the clip for free — no scissor, no extra copy, no UV-transform shader. `Fit` letterboxes the whole eye render instead, which is what you want when checking coverage at the frame edges. `vr.mirror_zoom` magnifies about the centre on top of either, for tightening the desktop view onto the middle of the eye's much wider field of view. The egui panel composite always fits at 1:1 — a cropped UI panel would lose its edges. The egui debug overlay composites onto the mirror before the present so it stays visible on the desktop. That overlay is rendered into a **window-sized** offscreen texture (`payload/src/hud/mirror_overlay.rs`) and composited across the whole back buffer, so it lands on the window at 1:1 whatever the render resolution is: the UI is measured in window pixels and is independent of the scene's. Sizing it to the back buffer instead couples it to the per-eye render resolution, which under `vr.own_back_buffer` off smears the UI horizontally as the near-square image is stretched onto a widescreen window, and under single-pass double-wide halves its relative width. Any draw/present fault disables `vr.mirror` at runtime and never wedges the loop; the window then holds its last frame.
...
- `resolution_scale` — per-eye render resolution scale on the runtime's recommendation (`1.0` = recommended).
...
- `own_back_buffer` — substitute a mod-owned render target for the engine's swapchain-derived back buffer, so the DXGI swapchain stays at the window size instead of following the per-eye render resolution (default on; see [`swapchain-ownership.md`](stereo/swapchain-ownership.md)). The mirror's present becomes 1:1, and single-pass double-wide stops forcing a 2x-width swapchain.
...
- Run the `coord_frame` diagnostic (`RUST_LOG=coord_frame=debug`) and walk forward to confirm the world frame before trusting HMD rotation (blocker 3, `docs/mod/body/head-and-body.md`).
...
- With `body_ik.enabled`, confirm the spine and neck bend toward where the player looks without fighting the head-bone override or the game's aim IK (`docs/engine/character/humanik.md` open risks); tune the reach weights in the VR/Camera tab.
