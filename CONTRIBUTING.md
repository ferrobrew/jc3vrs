...
- **Documentation**: The `docs/` directory contains reverse-engineered findings — reference as needed when working on specific systems.
...
- **Game shader reverse-engineering**: To extract, disassemble, and interpret the game's own shaders (the `.shader_bundle` files), and for how the mod patches them in memory, see `docs/shaders.md` and the tooling in `tools/shaders/`.
